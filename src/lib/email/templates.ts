    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e6dfd0;">
        <tr><td style="padding:24px 32px;border-bottom:1px solid #efe9da;">
          <div style="font-family:'Instrument Serif',Georgia,serif;font-size:24px;color:${PRIMARY};letter-spacing:-0.01em;">APRIM</div>
...
export function renderInquiryNotification(data: InquiryEmailData) {
  ...
  const body = `
    <p style="margin:0 0 16px 0;">A new patient inquiry was submitted on aprim.org.</p>
    ${detailsTable([
...
export function renderContactNotification(data: ContactEmailData) {
  ...
  const body = `
    <p style="margin:0 0 16px 0;">New contact submission via aprim.org.</p>
    ${detailsTable([
      ["Name", data.name],
      ["Email", data.email],
      ["Phone", data.phone],
...
export function renderInquiryConfirmation(data: { firstName: string }) {
  const subject = "We received your message — APRIM";
  const body = `
    ...
    <p style="margin:24px 0 0 0;">— The APRIM team</p>
  `;
...
export function renderPrescreenConfirmation(data: { firstName: string; studySlug: string }) {
  const subject = "Your pre-screening was received — APRIM";
  const body = `
    <p style="margin:0 0 16px 0;">Hi ${escapeHtml(data.firstName)},</p>
